...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  const requiredUserType = isClientRoute ? "client" : "admin";
  const redirectTo = isClientRoute ? "/client-login" : "/admin-login";
  ...
  const clientMenuItems = useMemo(() => ([
    { href: "/dashboard/client", icon: FaHome, label: "Overview", color: "text-blue-500" },
    { href: "/dashboard/client", icon: FaFileAlt, label: "Projects", color: "text-purple-500" },
    { href: "/dashboard/client", icon: FaUsers, label: "Team & Messages", color: "text-teal-500" },
    { href: "/dashboard/client", icon: FaBell, label: "Notifications", color: "text-yellow-500" },
    { href: "/dashboard/client", icon: FaCog, label: "Settings", color: "text-gray-500" },
  ]), []);

  const menuItems = user?.user_type === "client" ? clientMenuItems : adminMenuItems;
  ...
  return (
    <ProtectedRoute requiredUserType={requiredUserType} redirectTo={redirectTo}>
            ...
            >
              <div className="flex items-center justify-between p-4 border-b">
                <h2 className="text-xl font-bold text-gray-800">
                  {user?.user_type === "client" ? "Client Portal" : "Portfolio Admin"}
                </h2>
                <button
              ...
              <nav className="mt-4">
                ...
                <button
                  onClick={logout}
                  className="flex w-full items-center px-4 py-3 text-left text-gray-700 hover:bg-gray-100 transition-colors"
                >
            ...
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center">
                ...
                <h1 className="text-2xl font-bold text-gray-800">
                  {user?.user_type === "client" ? "Client Dashboard" : "Dashboard"}
                </h1>
              </div>
